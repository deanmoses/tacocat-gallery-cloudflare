...
Every push deploys through Workers Builds, Cloudflare's GitHub App: Cloudflare's build machines check out the commit, install the repo and run `scripts/release.sh`, the same script as by hand. A push to a pull request branch releases it to staging, so the branch can be looked at there before it is merged, and Cloudflare comments the link on the pull request. A merge to `main` releases production and staging at once, so staging is back on what production runs between pull requests. Staging does not go first: branch protection needs the branch up to date with `main`, so the tree being merged is the one its last push already released there. No API token is stored in GitHub; the build uses a token Cloudflare creates and holds. Each build's log and status are in the dashboard under the Worker, Settings, Builds, and Cloudflare posts a check run on the commit in GitHub.

The release is blue/green on Workers' versions and deployments. `scripts/release.sh` builds the web app and uploads a version, which serves no traffic; notes the database's Time Travel bookmark and applies the migrations, which are additive by rule so the version still serving keeps working; puts the new version in the deployment at 0% and checks it on the live hostname through the `Cloudflare-Workers-Version-Overrides` header (the health route answers with the version id it expects, the root album's JSON parses, the app shell is the app); then switches it to 100%, checks again, and rolls back if that fails. A failed check leaves the previous version serving and the build red. The script ends by printing the `wrangler rollback` command that undoes the release, which is safe as long as the migrations were additive.
...
Workers Builds does not wait for GitHub's checks. On `main` that is safe because branch protection lets nothing onto it that did not pass them; on a pull request branch it is the point, since staging is where the branch is looked at. The two connections are the one setting that lives in the dashboard rather than in this repo, since the GitHub App has to be authorized as a person, so here is what each is set to, for re-creating them:

| Setting                                  | Staging Worker                     | Production Worker                  | Why                                                                                                                                                                                                                                                              |
| ---------------------------------------- | ---------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Production branch                        | `main`                             | `main`                             |                                                                                                                                                                                                                                                                  |
| Root directory                           | `api`                              | `api`                              | Cloudflare checks that the Wrangler config in the root directory names the Worker being deployed.                                                                                                                                                                |
| Build command                            | `cd .. && npm ci --ignore-scripts` | `cd .. && npm ci --ignore-scripts` | The workspaces install from the repo root. `--ignore-scripts` skips the hook install and the Playwright browser download, which the build does not need.                                                                                                         |
| Deploy command                           | `../scripts/release.sh staging`    | `../scripts/release.sh production` | Runs for a push to the production branch. The root directory is `api`, hence the `../`.                                                                                                                                                                          |
| Non-production branch builds             | on, for every branch               | off                                | A pull request branch is looked at on staging. Off for production, where a branch build would touch the Worker readers use.                                                                                                                                      |
| Preview command                          | `../scripts/release.sh staging`    |                                    | Runs for a push to any other branch, in place of the deploy command. Its default, `npx wrangler preview`, would make a Worker Preview, an isolated copy of the Worker that the queue and the crons never reach, so the upload pipeline could not be tried on it. |
| Build watch paths                        | exclude `docs/**` and `**/*.md`    | exclude `docs/**` and `**/*.md`    | A docs-only push would otherwise run a whole release for nothing.                                                                                                                                                                                                |
| Build variable `SKIP_DEPENDENCY_INSTALL` | `1`                                | `1`                                | The root directory has no lockfile, so the automatic install would fail.                                                                                                                                                                                         |
| Build variable `HUSKY`                   | `0`                                | `0`                                |                                                                                                                                                                                                                                                                  |
| Build variable `NODE_VERSION`            | `24`                               | `24`                               | The `.nvmrc` is at the repo root, out of the root directory's view.                                                                                                                                                                                              |
