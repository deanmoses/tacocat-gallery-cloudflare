...
The workflow's Wrangler authenticates with the repository secret `CLOUDFLARE_API_TOKEN`, the only credential in GitHub: an account API token made in the Cloudflare dashboard (Manage account, Account API tokens, Create Token) from the Edit Cloudflare Workers template with D1 Edit and Containers Edit added, scoped to this account, and stored with `gh secret set CLOUDFLARE_API_TOKEN`. The same token is stored again with `gh secret set CLOUDFLARE_API_TOKEN --app dependabot`, since a Dependabot pull request's run reads only Dependabot's secrets, and without it a dependency bump would never reach staging before its merge released it to production. The account id is in `api/wrangler.jsonc`, so the token is all the workflow needs.
