...
**The web app** is an existing SvelteKit app ported to this back end, changed only where the platform forced it, so the API answers in the shapes it was written against. Which paths reach the Worker rather than the app's files is the `run_worker_first` list in `api/wrangler.jsonc`, so a new Worker route has to be added there; any other path with no file gets the app, which routes it in the browser.
