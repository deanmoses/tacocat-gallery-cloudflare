...
`web/` is the AWS app ported, not rewritten: `tacocat-gallery-sveltekit` at commit 8c57e5f of its `claude/getalbum-caching-nextprev-as8yq8` branch, with only what the platform forces changed: the URLs in `src/lib/utils/config.ts`, the session check and login page, and this repo's lint and test tooling. The Worker answers in the AWS API's record shapes (`shared/album.ts`) so the app parses them unchanged. Never rebuild a feature the AWS app already has; a difference between the two apps makes every performance comparison measure the apps instead of the platforms. The Worker has no search or admin write endpoints yet, so those parts of the app get a 404 and show their error state; when one is built, it answers in the AWS shape the app already sends and expects.
...
- **Deploy whenever testing a risk needs it.** Every push deploys itself: a pull request branch to staging, a merge to `main` to production and staging (Deploying in `README.md`). By hand, `scripts/release.sh staging` or `scripts/release.sh production`; a transcoder change needs `npm run deploy --workspace api` or `npm run deploy:production --workspace api` instead, the plain `wrangler deploy` that publishes the image. Say what went out. Requests to the site in the hour before an idle-probe run (00:23, 01:23, 03:23, 07:23 and 15:23 UTC) make that run less idle, and a merge's release checks production a few times; avoid those hours or note it in `docs/Perf.md`.
