...
- **Deploy whenever testing a risk needs it**, to staging with `npm run deploy --workspace api -- --containers-rollout=none` or to production with `npm run deploy:production --workspace api -- --containers-rollout=none`, dropping the flag only when the transcoder changed, and say what went out. Requests to the site in the hour before an idle-probe run (00:23, 01:23, 03:23, 07:23 and 15:23 UTC) make that run less idle; avoid those hours or note it in `docs/Perf.md`.
