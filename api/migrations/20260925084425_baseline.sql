...
CREATE TABLE `upload` (
	...
	CONSTRAINT "upload_path_check" CHECK(parent_path GLOB '/[0-9][0-9][0-9][0-9]/[0-9][0-9]-[0-9][0-9]/' AND item_name GLOB '?*.?*' AND item_name NOT GLOB '*.*.*' AND item_name NOT GLOB '*/*' AND lower(item_name) NOT GLOB '*.jpeg'),
	CONSTRAINT "upload_target_check" CHECK((target_path IS NULL AND target_id IS NULL) OR target_path IS NOT NULL AND substr(target_path, 1, 12) GLOB '/[0-9][0-9][0-9][0-9]/[0-9][0-9]-[0-9][0-9]/' AND substr(target_path, 13) GLOB '?*.?*' AND substr(target_path, 13) NOT GLOB '*.*.*' AND substr(target_path, 13) NOT GLOB '*/*' AND lower(substr(target_path, 13)) NOT GLOB '*.jpeg'),
...
CREATE TABLE `upload_error` (
	...
	`created_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
	`updated_at` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
	CONSTRAINT "upload_error_path_check" CHECK(path IS NOT NULL AND substr(path, 1, 12) GLOB '/[0-9][0-9][0-9][0-9]/[0-9][0-9]-[0-9][0-9]/' AND substr(path, 13) GLOB '?*.?*' AND substr(path, 13) NOT GLOB '*.*.*' AND substr(path, 13) NOT GLOB '*/*' AND lower(substr(path, 13)) NOT GLOB '*.jpeg'),
