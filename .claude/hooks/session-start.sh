...
# the npm bundled with Node 22 rewrites package-lock.json, stripping the `libc` fields that newer npm records for
# optional platform-specific packages (sharp has one per platform, through miniflare), so an unrelated change arrives
# with a few dozen lines of lockfile churn.
#
# Registered as a SessionStart hook in .claude/settings.json. The timeout there has to stay comfortably above the worst
# cold-start total (a cold image downloads a Node tarball before npm even starts), so bump both together if the
...
# A developer on their own machine manages their own Node; this is only for the cloud image, whose default is the one
# that cannot run the project.
...
# nvm installs itself somewhere different in every image, and it is a shell function rather than a binary, so it has
# to be sourced before it exists.
nvm_dir=""
...
if [ -z "$nvm_dir" ]; then
    ...
    if [ "$actual_major" != "$wanted_major" ]; then
        echo ".nvmrc wants Node $wanted_major, and engine-strict makes npm install refuse any other. Skipping it."
        exit 0
    fi
else
...
# install rather than ci: the container image is cached after this hook, and install can reuse what is already
# unpacked there.
